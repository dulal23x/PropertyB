...
export default function AdminPropertiesPage() {
  ...
  const runSearch = (page = 1) => {
    ...
    setQuery(buildQuery(newFilters));
  };
  ...
  const runBulkAction = async (action: "approve" | "reject" | "unpublish" | "archive") => {
    ...
    try {
      let res;
      if (action === "approve") {
        res = await bulkApproveListings(selectedIds);
      } else if (action === "reject") {
        const note = window.prompt(`Reject ${selectedIds.length} listings. Note required:`);
        if (!note) return;
        res = await bulkRejectListings(selectedIds, note);
      } else if (action === "unpublish") {
        const note = window.prompt(`Unpublish ${selectedIds.length} listings. Optional note:`);
        res = await bulkUnpublishListings(selectedIds, note || undefined);
      } else {
        if (!window.confirm(`Archive ${selectedIds.length} listings?`)) return;
        res = await bulkArchiveListings(selectedIds);
      }
      setMessage(`Bulk ${action} completed. Affected: ${res.count} listings.`);
      await load();
    } catch (err) {
    ...
    <div className="space-y-6 rounded-3xl border border-white/10 bg-white p-5 text-slate-900 shadow-2xl">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex gap-3">
          <button onClick={() => runSearch(1)} className="rounded-2xl bg-emerald-600 px-4 py-2 text-sm font-black text-white shadow-md hover:bg-emerald-700">
            Apply filters
          </button>
          <button onClick={() => { setFilters(initialFilters); setQuery("status=pending_review"); }} className="rounded-2xl border border-slate-200 px-4 py-2 text-sm font-black hover:bg-slate-50">
            Reset
          </button>
        </div>

        {selectedIds.length > 0 && (
          <div className="flex items-center gap-2 rounded-2xl bg-slate-100 p-2 pl-4">
            <span className="text-xs font-bold text-slate-600">{selectedIds.length} selected</span>
            <div className="flex gap-1">
              <button onClick={() => runBulkAction("approve")} className="rounded-xl bg-emerald-600 px-3 py-1.5 text-[10px] font-black text-white">Approve</button>
              <button onClick={() => runBulkAction("reject")} className="rounded-xl bg-rose-600 px-3 py-1.5 text-[10px] font-black text-white">Reject</button>
              <button onClick={() => runBulkAction("unpublish")} className="rounded-xl bg-amber-500 px-3 py-1.5 text-[10px] font-black text-white">Unpublish</button>
              <button onClick={() => runBulkAction("archive")} className="rounded-xl bg-slate-700 px-3 py-1.5 text-[10px] font-black text-white">Archive</button>
            </div>
          </div>
        )}
      </div>
              ...
              <thead>
                <tr className="text-xs uppercase tracking-[0.2em] text-slate-500">
                  <th className="px-3 py-2">
                    <input type="checkbox" checked={selectedIds.length === items.length && items.length > 0} onChange={toggleSelectAll} className="h-4 w-4 rounded border-gray-300 text-emerald-600 focus:ring-emerald-500" />
                  </th>
                ...
                {items.map((item) => (
                  <tr key={item.id} className="rounded-2xl bg-slate-50">
                    <td className="px-3 py-3">
                      <input type="checkbox" checked={selectedIds.includes(item.id)} onChange={() => toggleSelect(item.id)} className="h-4 w-4 rounded border-gray-300 text-emerald-600 focus:ring-emerald-500" />
                    </td>
                    ...
                    <td className="px-3 py-3">
                      <div className="flex flex-wrap gap-2">
                        <Link href={`/admin/properties/${item.id}`} className="rounded-xl bg-white px-3 py-2 text-xs font-black text-emerald-700 shadow-sm hover:bg-slate-50">
                          Review
                        </Link>
                        <button onClick={() => quickAction(item.id, "approve")} className="rounded-xl bg-emerald-600 px-3 py-2 text-xs font-black text-white hover:bg-emerald-700">
                          Approve
                        </button>
                        <button onClick={() => quickAction(item.id, "reject")} className="rounded-xl bg-rose-600 px-3 py-2 text-xs font-black text-white hover:bg-rose-700">
                          Reject
                        </button>
                        <button onClick={() => quickAction(item.id, "unpublish")} className="rounded-xl bg-amber-500 px-3 py-2 text-xs font-black text-white hover:bg-amber-600">
                          Unpublish
                        </button>
                        <button onClick={() => quickAction(item.id, "archive")} className="rounded-xl bg-slate-700 px-3 py-2 text-xs font-black text-white hover:bg-slate-800">
                          Archive
                        </button>
                      </div>
                    </td>
          ...
          <div className="mt-4 flex items-center justify-between border-t border-slate-100 pt-4">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="rounded-xl border border-slate-200 px-3 py-1 text-sm font-bold disabled:opacity-50 hover:bg-slate-50"
              >
                Prev
              </button>
