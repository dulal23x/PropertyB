...
export default function RevealContactButton() {
  ...
  const handleReveal = async () => {
    ...
    setError(false);
    try {
      const data = await apiFetch<{ contact_number: string }>("/properties/global-contact");
      setNumber(data.contact_number || "+8801700000000");
    } catch (err) {
      console.error(err);
      setError(true);
    } finally {
    ...
    <div className="space-y-2">
      ...
      {error && (
        <p className="text-xs text-red-600 text-center font-medium">
          Unable to load number. Please use the form.
        </p>
      )}
    </div>
  );
