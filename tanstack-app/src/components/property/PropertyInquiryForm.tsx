...
export default function PropertyInquiryForm({ listingId }: PropertyInquiryFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await apiFetch(`/properties/${listingId}/inquiries`, {
        method: "POST",
        body: JSON.stringify({
          name,
          phone,
    ...
    <form className="space-y-4" onSubmit={handleSubmit}>
      ...
      <div>
        <input
          ...
          required
          className="w-full border border-gray-300 px-3 py-2 rounded-md shadow-sm focus:outline-none focus:ring-1 focus:ring-brand-green"
        />
      </div>
      <div>
        <input
      ...
      <button
        type="submit"
        disabled={loading}
        className="w-full bg-brand-dark text-white font-bold py-3 rounded-md hover:bg-gray-800 transition-colors disabled:opacity-60"
      >
