      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex h-16 items-center justify-between gap-2">
          <div className="flex min-w-0 items-center">
            <a href="/" aria-label="PropertyBikri home" className="flex-shrink-0 flex items-center">
              <img
            ...
            <nav className="hidden xl:ml-5 xl:flex xl:items-center xl:gap-1 2xl:ml-7 2xl:gap-2">
              {mainMenuItems.map((item) => (
                <a
                  key={item.href}
                  href={item.href}
                  className="whitespace-nowrap px-1.5 py-2 text-[11px] font-black uppercase tracking-normal text-brand-dark transition-colors hover:text-brand-green 2xl:px-2 2xl:text-[12px]"
                >
                  {item.shortLabel}
                </a>
              ))}
          ...
          <div className="flex shrink-0 items-center gap-2 sm:gap-3">
            ...
            >
              ...
              <span className="hidden whitespace-nowrap text-[11px] font-black uppercase tracking-normal xl:inline 2xl:text-[12px] 2xl:tracking-widest">
                {accountLabel}
              </span>
            </a>
              ...
              <div className="flex flex-col gap-2 px-5 py-5">
                {mainMenuItems.map((item) => (
                  <a
                    ...
                    onClick={closeMobileMenu}
                    href={item.href}
                    className="rounded-xl border border-gray-100 px-4 py-3 text-sm font-bold uppercase tracking-wider text-brand-dark"
                  >
                    {item.label}
                  </a>
                ))}
                <a
                  onClick={closeMobileMenu}
                  href={postPropertyHref}
                  className="mt-2 rounded-xl bg-brand-green px-4 py-3 text-sm font-black uppercase tracking-wider text-white text-center"
                >
                  Post Property
                </a>
                <a
                  onClick={closeMobileMenu}
                  href={accountHref}
                  className="rounded-xl border border-gray-100 px-4 py-3 text-sm font-bold uppercase tracking-wider text-brand-dark text-center"
                >
                  {accountLabel}
                </a>
              </div>
